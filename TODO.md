...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **TSK-001** | Phase 1 | Initialisation de la solution MVC (.NET 8.0, structure des dossiers) | - | P0 | `Backlog` | Agent | 2026-07-02 | Aucun | `Program.cs`, `.csproj` |
| **TSK-002** | Phase 1 | Schéma de base de données (Entités EF Core, Context, configurations) | DONNÉES-01 | P0 | `Backlog` | Agent | 2026-07-02 | Aucun | `ApplicationDbContext.cs` |
| **TSK-003** | Phase 1 | Contrainte d'unicité SQL sur `BoxPackages.PackageBarcode` | SCAN-03 | P0 | `Backlog` | Agent | 2026-07-02 | Clé pour l'intégrité des données | EF Core Fluent API |
...
| **TSK-014** | Phase 3 | Clôture automatique de box (quantité attendue atteinte) | SCAN-06 | P1 | `Backlog` | Agent | 2026-07-02 | Transition vers statut "Completed" | `IScanService` / `Box` |
| **TSK-015** | Phase 3 | Gestion des scans concurrents et de la concurrence optimiste | - | P0 | `Backlog` | Agent | 2026-07-02 | Gérer DbUpdateConcurrencyException | EF Core `RowVersion` |
| **TSK-016** | Phase 4 | Journal d'audit append-only immuable | AUDIT-01, AUDIT-02 | P0 | `Backlog` | Agent | 2026-07-02 | SaveChangesInterceptor pour EF Core | `BoxAuditLogInterceptor` |
| **TSK-017** | Phase 4 | Reprise de box ouverte par un autre opérateur | BOX-04 | P1 | `Backlog` | Agent | 2026-07-02 | Conserver créateur initial | `IBoxService` |
| **TSK-018** | Phase 4 | Clôture exceptionnelle avec écart (Superviseur uniquement + motif) | EXC-02 | P1 | `Backlog` | Agent | 2026-07-02 | CompletedWithException + CompletionMode=Forced | `BoxController.cs` |
| **TSK-019** | Phase 4 | Annulation de box (Superviseur uniquement + motif) | EXC-01 | P1 | `Backlog` | Agent | 2026-07-02 | Statut Cancelled | `BoxController.cs` |
| **TSK-020** | Phase 4 | Blocage et déblocage de boxes / packages (Quarantaine) | EXC-04 | P1 | `Backlog` | Agent | 2026-07-02 | Statut Blocked | `BoxController.cs` |
| **TSK-021** | Phase 4 | Retrait, transfert et désaffectation contrôlés de packages | EXC-03 | P1 | `Backlog` | Agent | 2026-07-02 | Traçabilité avec motif obligatoire | `IBoxService` |
| **TSK-022** | Phase 4 | Recherche multicritères et filtres (Statut, numéro, date, user) | - | P2 | `Backlog` | Agent | 2026-07-02 | Pour superviseurs et admins | `BoxController.cs` |
| **TSK-023** | Phase 5 | Tests unitaires et d'intégration métier | - | P1 | `Backlog` | Agent | 2026-07-02 | Couvrir règles d'unicité et concurrence | Projet de tests |
| **TSK-024** | Phase 5 | Audit de sécurité (Vérification secrets, logs et injection SQL) | - | P0 | `Backlog` | Agent | 2026-07-02 | Checklist sécurité | Analyse statique de code |
| **TSK-025** | Phase 5 | Préparation de la configuration de déploiement local (IIS / Kestrel) | - | P2 | `Backlog` | Agent | 2026-07-02 | Aucun | `appsettings.json` |
